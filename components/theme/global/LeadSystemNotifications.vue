<template>
  <div :class="['notification', showList ? 'show' : '']">
    ...
    <div v-click-outside="onClickOutsideOfList" class="notification-body">
                      ...
                      <a
                        ...
                        @click="
                          taskClicked({
                            ...
                            lead_id: noti.lead_detail.id,
                            task_id: noti.task_detail.id,
                            notification_id: noti.id,
                          })
                        "
                      >
                        {{ noti.task_detail.title }}
                      </a>
                      <a
                        v-else
                        href="javascript:void(0)"
                        ...
                        "
                      >
                        {{ noti.lead_detail.first_name }}
                        {{ noti.lead_detail.last_name }}
                      </a>
                      <div class="path-link">
      ...
      <div v-if="false" class="bottom-fixed">
        <nuxt-link
          :to="{
...
export default {
  ...
  data() {
    return {
      ...
      badgesCount: this.$auth.user.lead_badges_count,
  ...
  mounted() {
    this.$echo
      ...
      .listen('.ProjectUserNotificationBadgesEvent', (e) => {
        // console.log(e)
        const {
          data: { lm_badges_count },
        } = e
        this.badgesCount = lm_badges_count
        // this.closeList()
      })
    this.loadJQ()
  ...
  methods: {
    async getBadgesCount() {
      try {
        ...
        this.badgesCount = data.lm_badges_count
      } catch (e) {
    ...
    clicked(noti) {
      ...
      if (noti.task_detail) {
        ...
        this.leadClicked(params)
      }
    },
    taskClicked(params) {
      const { workspace_id, lead_id, notification_id } = params
      this.closeList()
    ...
    async initialLoadNotifications() {
      try {
        ...
        const { data } = await this.$axios.$get(
          `user-notification/lead?page=${this.page}&module_id= 3`
        )
    ...
    async readNotification(nId) {
      ...
      if (findIndex !== -1) {
        ...
        if (read_at === null) {
          ...
          try {
            await this.$axios.$post(`user-notification/lead-read`, {
              notification_id: nId,
            })
            // this.unreadNotifications = data.total_unread_notification
            // console.log(data)
    ...
    async readUnreadNotification(nId) {
      ...
      try {
        await this.$axios.$post(`user-notification/lead-read-unread`, {
          notification_id: nId,
        })
        // this.unreadNotifications = data.total_unread_notification
        // console.log(data)
    ...
    async readAllNotifications() {
      ...
      try {
        await this.$axios.$post(`user-notification/mark-all-lead-read`, {})
        // this.unreadNotifications = data.total_unread_notification
      } catch (e) {
        this.$toast.error(this.$getErrorMessage(e))
      }
    ...
    async initialLoadAnnouncements() {
      try {
        ...
        this.badgesCount = 0
        const { data } = await this.$axios.$get(
          `announcement/notification-list?page=${this.page_}&module_id= 3`
        )
    ...
    async readAnnouncement(aId) {
      ...
      if (findIndex !== -1) {
        ...
        if (read_at === null) {
          ...
          try {
            await this.$axios.$post(`announcement/read`, {
              notification_id: aId,
              module_id: 3,
            })
            // this.unreadNotifications = data.total_unread_notification
    ...
    async readUnreadAnnouncement(aId) {
      ...
      try {
        await this.$axios.$post(`announcement/read-unread`, {
          notification_id: aId,
          module_id: 3,
        })
        // this.unreadNotifications = data.total_unread_notification
    ...
    async readAllAnnouncements() {
      ...
      try {
        await this.$axios.$post(`announcement/mark-all-read?module_id= 3`)
        // this.unreadNotifications = data.total_unread_notification
      } catch (e) {
        this.$toast.error(this.$getErrorMessage(e))
      }
