            ...
            <ul class="thead">
              <li>
                <input
                  ...
                  class="form-control"
                  name="search"
                  placeholder="Search Lead Task status"
                  data-lpignore="true"
                />
              </li>
            </ul>
                  ...
                  >
                    <input
                      ...
                      :disabled="
                        ...
                        (parseInt(leadStatus.lead_task_count) > 0 && true)
                      "
                      type="checkbox"
...
export default {
  ...
  methods: {
    async deleteStatus(status_id) {
      try {
        const res = await this.$axios.$post('/lm/lead-task-status/delete', {
    ...
    async getLeadStatusList() {
      ...
      try {
        const lead_status = await this.$axios.$post(
          `/lm/lead-task-status/list`,
          {
    ...
    async dragLeadStatus() {
      ...
      try {
        const { message } = await this.$axios.$post(
          `/lm/lead-task-status/change-ordering`,
          {
            workspace_id: this.internal_workspace_id,
