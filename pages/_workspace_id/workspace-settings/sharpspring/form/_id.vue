...
export default {
  components: { ClientSelect2, Select2, ContentLoader, draggable },
  ...
  async mounted() {
    ...
    await this.getStartSpringFields()
  ...
  methods: {
    async getFormData() {
      try {
        const { data } = await this.$axios.$post(`lm/sharpspring/view`, {
          workspace_id: this.internal_workspace_id,
          sharp_spring_forms_id: this.$route.params.id,
        })
        this.formTitle = data.title
        this.sharpSpringFormId = data.form_id
        this.status = data.status
        this.fields = data.fields_data
      } catch (e) {
    ...
    async submitForm() {
      ...
      try {
        this.loading = true
        await this.$axios.$post(`lm/sharpspring/update`, {
          sharp_spring_forms_id: this.$route.params.id,
          workspace_id: this.internal_workspace_id,
        ...
        this.loading = false
        this.$toast.success(
          `<strong>${this.formTitle}</strong>&nbsp;is successfully updated`
        )
